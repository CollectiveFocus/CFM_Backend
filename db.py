...
class FridgeHistory(DB_Item):
    ...
    "epochTimestamp": {"required": True, "type": "N"},
    "fridge_json_data": {"required": True, "type": "S"}
    }
    REQUIRED_FIELDS = ["fridgeId", "fridge_json_data"]
    TABLE_NAME = f"fridge_history_{DB_Item.STAGE}"

    def __init__(self, db_client: "botocore.client.DynamoDB", fridge_history: dict = None):
        super().__init__(db_client=db_client)
        if fridge_history is not None:
            self.fridgeId: str = fridge_history.get("fridgeId", None)
            self.epochTimestamp: str = fridge_history.get(
                "epochTimestamp", None)
            self.fridgeJsonData = json.dumps(fridge_history.get("fridgeJsonData", None))
