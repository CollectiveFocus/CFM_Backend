...
TEST_DB_RESPONSE_FAILED = {"ResponseMetadata": {"HTTPStatusCode": 400}}
...
def test_format_api_response_success():
    ...
    response = handler.format_api_response(
        db_response=TEST_DB_RESPONSE_SUCCESS, response_type="Items"
    )
    assert response == {
        "statusCode": 200,
        "headers": {
...
def test_format_api_response_failed():
    handler = GetAllFridgesHandler(env="", table_name="", ddbclient={})
    response = handler.format_api_response(
        db_response=TEST_DB_RESPONSE_FAILED, response_type="Items"
    )
    assert response == {
        "statusCode": 400,
        "headers": {
