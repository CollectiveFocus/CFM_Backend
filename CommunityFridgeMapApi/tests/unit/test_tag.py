...
class TagTest(unittest.TestCase):
    def test_is_valid_tag_name(self):
        ...
        expected_equal = [
            (
                Tag.is_valid_tag_name("")[1],
                "Length of tag_name is 0. It should be >= 3 but <= 32.",
            ),
            (Tag.is_valid_tag_name(None)[1], "Missing required fields: tag_name"),
            (
                Tag.is_valid_tag_name("ab")[1],
                "Length of tag_name is 2. It should be >= 3 but <= 32.",
            ),
            (
                Tag.is_valid_tag_name("test&tag")[1],
                "tag_name contains invalid characters",
            ),
            (
